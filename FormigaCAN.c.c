...
//Configuração dos pinos para comunicação CAN
CAN can0(p196, p194)
...
/*    
    Envio de mensagens
        ...
        if(can0.write(CANMessage(1111, &dado , 12))){
    }

    1111: identificador, &dado: endereço de início dos dados, 2: bytes a serem enviados
...
*/    
	...
	while (MatrizControle[0][0] <= 10){
		MatrizElevacao[i][j] += MatrizControle[0][0]*2;
		        if(can0.write(CANMessage(1111, &MatrizElevacao[i][j] , 12))){
		            		MatrizControle[0][0]++;
        }
	}
	// CAN
...
void MoverPernaRev(int i, int j) {
	...
	while (MatrizControle[1][0] <= 30){
		MatrizRotacao[i][j] -= MatrizControle[1][0];
		        if(can0.write(CANMessage(1111, &MatrizRotacao[i][j] , 12))){
		            		MatrizControle[1][0]++;
        }
	}
	// CAN
